...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        # Load the image
        img_data = io.loadmat(folder + 'Houston13.mat')
        img = np.asarray(img_data['ori_data'])  # .transpose(1, 2, 0)
        # print(img.shape)
        gt_data = io.loadmat(folder + 'Houston13_7gt.mat')
        gt = np.asarray(gt_data['map'])
        # print(gt.shape)
        # img = np.asarray(open_file(folder + 'Houston13.mat')
        #                  ['ori_data']).transpose(1, 2, 0)

        rgb_bands = [13, 20, 33]  # 包含图像中红、绿、蓝波段的索引值
        ...
        # Load the image
        img_data = io.loadmat(folder + 'Houston18.mat')
        img = np.asarray(img_data['ori_data'])  # .transpose(1, 2, 0)
        gt_data = io.loadmat(folder + 'Houston18_7gt.mat')
        gt = np.asarray(gt_data['map'])

        # img = np.asarray(open_file(folder + 'Houston18.mat')
        #                  ['ori_data']).transpose(1, 2, 0)
        # img = io.loadmat(folder + 'Houston13.mat')['ori_data']
        # gt = io.loadmat(folder + 'Houston13.mat')['map']
        rgb_bands = [13, 20, 33]
        ...
        # Load the image
        # img = np.asarray(open_file(folder + 'paviaU.mat')
        #                  ['ori_data']).transpose(1, 2, 0)
        img_data = io.loadmat(folder + 'paviaU.mat')
        img = np.asarray(img_data['ori_data'])  # .transpose(1, 2, 0)

        gt_data = io.loadmat(folder + 'paviaU_7gt.mat')
        gt = np.asarray(gt_data['map'])
        ...
        # Load the image
        img_data = io.loadmat(folder + 'paviaC.mat')

        img = np.asarray(img_data['ori_data'])  # .transpose(1, 2, 0)

        gt_data = io.loadmat(folder + 'paviaC_7gt.mat')

        gt = np.asarray(gt_data['map'])

        # img = np.asarray(open_file(folder + 'paviaC.mat')
        #                  ['ori_data']).transpose(1, 2, 0)
        ...
        img_data = io.loadmat(folder + 'Dioni.mat')
        img = np.asarray(img_data['ori_data'])
        gt_data = io.loadmat(folder + 'Dioni_gt_out68.mat')
        gt = np.asarray(gt_data['map'])
        ...
        img_data = io.loadmat(folder + 'Loukia.mat')
        img = np.asarray(img_data['ori_data'])

        gt_data = io.loadmat(folder + 'Loukia_gt_out68.mat')
        gt = np.asarray(gt_data['map'])
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt, transform=None, **hyperparams):
        ...
        p = self.patch_size // 2
        self.indices = np.array([(x, y) for x, y in zip(
            x_pos, y_pos) if x > p and x < data.shape[0] - p and y > p and y < data.shape[1] - p])
...
if __name__ == "__main__":
    # 示例高光谱图像数据
    H, W, C = 100, 100, 30  # H: 高度, W: 宽度, C: 通道数（光谱）
    data = np.random.rand(H, W, C)  # 生成随机高光谱数据
    gt = np.random.randint(0, 5, size=(H, W))  # 生成随机标签 (0 到 4 类)

    # 超参数设置
    hyperparams = {
        'patch_size': 13,  # Patch 的大小
        'ignored_labels': [0],  # 忽略的标签 (例如背景为 0)
