...
# 设置随机数种子并配置一些选项，以确保 PyTorch 和其他库中的随机过程可复现。在深度学习实验中，使用相同的种子可以让每次运行的实验结果一致，便于调试和结果验证。
    ...
    if mode == 'random':
        if train_size == 1:
            ...
            train_indices, test_indices = sklearn.model_selection.train_test_split(
                X, train_size=train_size, stratify=y, random_state=23)
            ...
            test_set = np.column_stack(
                (test_indices[0], test_indices[1], test_label))
