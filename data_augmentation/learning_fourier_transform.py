...
class FourierTransformModule(nn.Module):
    def __init__(self, split_ratio=0.5, a=0.5, b=0.8, beta_mean=0.1, beta_std=0.05):
        super(FourierTransformModule, self).__init__()

        # 初始化可学习参数，使用 nn.Parameter
        # 需要将这些参数限制在合理的范围内，所以使用了 clamp 来约束范围
        self.split_ratio = nn.Parameter(
            torch.tensor(split_ratio, dtype=torch.float32))
    ...
    def forward(self, X):
        ...
        # 使用 sigmoid 将 split_ratio 约束在 [0, 1] 区间
        split_ratio = torch.sigmoid(self.split_ratio)

        # 使用 clamp 限制 a 和 b 的范围
        a = torch.clamp(self.a, min=0.0, max=1.0)
        b = torch.clamp(self.b, min=a.item(), max=1.0)

        # beta_mean 和 beta_std 通过 sigmoid 和 relu 限制在合理范围内
        # beta_mean = torch.sigmoid(self.beta_mean)  # 限制在 [0, 1]
        # beta_std = torch.relu(self.beta_std)  # 限制在 [0, +∞)
